...
In comparison to most other widely adopted languages the C++ language provides a very expressive type system\todo{cite}. Custom
types (\lstinline{structs} and \lstinline{classes}) do not only consist of other types but can also depend on other
types and values. This yields types which can fundamentaly change their behaviour based on other types\todo{cite std}. As a result
...
can be computed can also be computed by the C++ type system \todo{cite turing complete}.
...
is used for the definition of Turing computability\todo{cite turing}.
